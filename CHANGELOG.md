...
- [rd-writer] An `\item` option was previously silently dropped and surfaced as `WriteError::Verification` without an AST location. It is now rejected as `WriteError::Unserializable`; R rejects `\item[...]` in the two-group contexts and treats the brackets as ordinary sibling text in itemize context, so the shape is unrepresentable.
