...
record layer recognizes uncompressed and zlib records. Bzip2 and LZMA entries
...
The `lazyload` feature enables only the record zlib dependency. It does not
enable the standalone `.rds` gzip envelope feature; callers that need both
must request `features = ["lazyload", "gzip"]`.
