...
fn generation_header_matches_realistic_preamble_shape() {
    ...
    let view = document.generation_header_lossy().unwrap();
...
fn generation_header_reads_wrapped_and_crlf_preambles() {
    ...
    assert_eq!(
        unix.generation_header_lossy().unwrap().source_files(),
        &["R/a.R", "R/b.R"]
    );

    let crlf = parse("% Please edit documentation in R/a.R,\r\n%   R/b.R\r\n\\name{x}\\title{t}");
    assert_eq!(
        crlf.generation_header_lossy(),
        unix.generation_header_lossy()
    );
}
...
fn lifecycle_badge_view_reads_real_rd() {
    ...
    let bare_badges = bare.lifecycle_badges_lossy();
