...
fn recognizes_marker_and_sources() {
    ...
    let view = doc.generation_header_lossy().unwrap();
...
fn equality_ignores_provenance_positions() {
    ...
    let first = first_document.generation_header_lossy().unwrap();
    let second = second_document.generation_header_lossy().unwrap();
...
fn preserves_source_order_and_wrapped_files() {
    ...
    assert_eq!(
        doc.generation_header_lossy().unwrap().source_files(),
...
fn marker_and_source_presence_are_independent() {
    ...
    let marker = marker_document.generation_header_lossy().unwrap();
    ...
    let source = source_document.generation_header_lossy().unwrap();
...
fn scans_only_the_leading_region_and_ignores_nested_comments() {
    ...
    assert!(document(vec![]).generation_header_lossy().is_none());
    assert!(
...
fn stops_after_non_header_content_and_rejects_near_misses() {
    ...
    assert_eq!(
        unknown.generation_header_lossy().unwrap().generator(),
    ...
    assert!(partial.generation_header_lossy().unwrap().is_generated());
}
